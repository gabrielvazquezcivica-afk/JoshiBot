...
export const handler = async (m, { sock, from, isGroup, sender, reply, args, owner }) => {
  ...
  if (!text) return reply('❌ Ingresa el texto para crear el sticker.\nEjemplo: `.brat Hola mundo`')
  ...
  try {
    ...
    if (!res.data || res.data.byteLength === 0) throw new Error('La API devolvió datos vacíos')

    // Convertir a WebP válido con EXIF
    const stickerBuffer = await addExif(res.data, 'JoshiBot', sender.split('@')[0])
